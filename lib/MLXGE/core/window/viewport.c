...
/*   Updated: 2023/12/26 13:28:39 by lmells           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void	push_viewport_list_back(t_viewport *view,
						t_viewport **list)
{
	t_viewport	*node;
				...
				t_dimensions size)
{
	...
	if (view)
	{
		...
		{
			mlxge_log(ERROR, ERR_VIEW_CREAT" : "\
			"There is no update frame to draw graphics");
			free(view);
			view = (void *)0;
		}
	}
...
t_cam_ortho2d	*mlxge_new_camera_2d_orthographic(t_v2i origin)
{
	...
	{
		mlxge_log(ERROR,
			"Failed to create a new MLXGE 2D orthographic camera because : "\
