...
public func mlx_new_window_swift(_ mlxptr:UnsafeRawPointer, Width w:UInt32, Height h:UInt32, Title t:UnsafePointer<CChar>) -> UnsafeRawPointer
{
		...
		// --- Added by lmells98 (42-Adelaide) ---------------------------------
		let frame = mw.getScreenFrame()
		let mwFrame = mw.getWinEFrame()
		let titleBarHeight = mw.getWinETitleBarHeight()

		let centerX = CGFloat((frame.size.width - mwFrame.size.width) / 2)
		let centerY = CGFloat((frame.size.height - mwFrame.size.height) / 2 + titleBarHeight)
		mw.setFramePostition(rect: NSRect(x: centerX, y: centerY, width: CGFloat(w), height: CGFloat(h)));
		// ---------------------------------------------------------------------
